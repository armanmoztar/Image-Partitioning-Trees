...
Node* BuildNodeCopy(Node* node);

HSLAPixel BuildNodeHelper(PNG& im, pair<unsigned int, unsigned int> ul, unsigned int w, unsigned int h);

void render(Node* node, PNG& png) const;

int SizeLoc(Node * root) const;

void Clear(Node* root);

int Leaves(Node * root, int leaves) const;

void prune(Node* node, double tolerance);

bool IsPrunable(Node* og, Node* root, double tolerance);
